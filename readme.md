...
- **Performance is currently slower than pure-Ruby `gsub`.** A May 2026 investigation found the C extension is 4–7× slower than a pure-Ruby `gsub` loop running the same 88 patterns, across input sizes from 168 bytes to 1 MB. The cause is glibc's POSIX `regexec()`: each call allocates an O(input-length) state buffer before any matching begins, and calling it in a loop across 88 patterns × M matches is mathematically O(N²) per redaction. This is not a bug in the gem's calling code — the buffer-sizing path *was* O(N²) (fixed in commit `7a70f0a`), but the dominant cost is per-`regexec` setup, which can only be addressed by skipping pattern invocations (a `memchr` literal pre-filter — see TODO) or replacing the engine entirely (see `docs/standalone_matcher_design.md`). Use the gem on small payloads where the absolute latency is still acceptable (< 1 ms for typical log lines); for high-throughput pipelines, hold off until the next major release.

# bench5.rb — Option A: 88-pattern AC + Onigmo (libonig-dev, no Ruby VM).
#
# Usage:
#   make matcher5.so
#   bundle exec ruby bench5.rb verify    # correctness vs DataRedactor.scan
#   bundle exec ruby bench5.rb bench     # benchmark
#   bundle exec ruby bench5.rb           # both
...
# ---------- FFI binding -----------------------------------------------------
#
# Load matcher5.so with RTLD_DEEPBIND so that its onig_* calls resolve to
...
SO_PATH = File.join(PROTOTYPE_DIR, "matcher5.so")

# RTLD_DEEPBIND (8) makes matcher5.so resolve its own onig_* symbols against
...
# return garbage.
MM5_HANDLE = Fiddle::Handle.new(SO_PATH, Fiddle::Handle::RTLD_NOW | 8)

module MM5
  MM5_Init = Fiddle::Function.new(MM5_HANDLE["mm5_init"], [], Fiddle::TYPE_VOID)
  MM5_Free = Fiddle::Function.new(MM5_HANDLE["mm5_free"], [], Fiddle::TYPE_VOID)
  MM5_Scan = Fiddle::Function.new(
    MM5_HANDLE["mm5_scan"],
    [Fiddle::TYPE_VOIDP, Fiddle::TYPE_SIZE_T, Fiddle::TYPE_VOIDP, Fiddle::TYPE_SIZE_T],
    Fiddle::TYPE_SIZE_T)

  def self.mm5_init = MM5_Init.call
  def self.mm5_free = MM5_Free.call
  def self.mm5_scan(input, len, buf, max) = MM5_Scan.call(input, len, buf, max)
end

# struct mm5_match_t layout (64-bit):
...
def mm5_scan_ruby(input)
  MM5.mm5_init
  max = 65536
  buf = Fiddle::Pointer.malloc(MATCH_STRIDE * max, Fiddle::RUBY_FREE)
  n = MM5.mm5_scan(input, input.bytesize, buf, max)
  n.times.map do |i|
    base = i * MATCH_STRIDE
...
def verify
  ...
  VERIFY_CASES.each_with_index do |s, i|
    proto = mm5_scan_ruby(s).map { |m| [m[:name], m[:start], m[:length]] }.sort
...
def bench
  ...
  t_proto = Benchmark.realtime { iters.times { mm5_scan_ruby(payload) } }
  ...
  puts "Prototype v5 (AC+Onigmo+BM): #{(t_proto / iters * 1000).round(1)} ms/iter"
  puts
  ratio = t_pure / t_proto
  puts "v5 vs pure-Ruby : #{ratio.round(2)}× #{ratio >= 1.0 ? "(beats Ruby!)" : "(slower than Ruby)"}"
  puts "v5 vs today's C : #{(t_dr / t_proto).round(2)}×"
ensure
  MM5.mm5_free
end

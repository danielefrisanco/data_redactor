/* matcher17.c — v17: v15.1 + precomputed initial thread list.
 *
 * At every candidate position, scan_one calls addthread(pc=0) to epsilon-close
 * the program from the start. For patterns without BOL/EOL anchors the closure
 * result is position-independent — the same set of byte-consuming pcs every time.
 * v17 computes this set once at init and stores it as eng->init_list[]. At seed
 * time we memcpy that list into cl instead of running the DFS. This eliminates
 * the most frequent addthread call (the per-position seed) for ~85 of 88 patterns.
 *
 * Patterns that have BOL anchors need a position-sensitive closure and fall back
 * to calling addthread as before.
 *
 * Build:  make matcher17.so   (shared lib)
 *         make matcher17       (self-test binary)
 */
...
typedef struct {
    ...
    /* v17: precomputed initial thread list — epsilon closure of pc=0 when the
     * start position is not BOL. For patterns without BOL anchors this is
     * position-independent and computed once at init. has_bol=1 means the
     * pattern uses ^, so we fall back to addthread at seed time. */
    int        *init_list;
    int         init_n;
    int         init_matched;
    int         has_bol;
...
/* Forward declaration — addthread is defined after mm17_init. */
static void addthread(prog_t *pr, tlist_t *tl, int *seen, int gen,
                      int *stk, int pc0,
                      const char *input, size_t len, size_t pos);

/* Returns 1 if the program contains any OP_BOL instruction. */
static int prog_has_bol(const prog_t *pr) {
    for (int i = 0; i < pr->n; i++)
        if (pr->code[i].op == OP_BOL) return 1;
    return 0;
}
...
void mm17_init(void) {
    ...
    for (int p = 0; p < MM88_NUM_PATTERNS; p++) {
        ...
        /* v17: precompute the epsilon closure of pc=0 for a non-BOL position.
         * We run addthread with a temporary seen[] and stack, storing the
         * resulting thread list in eng->init_list. Patterns with BOL anchors
         * need a position-sensitive closure and skip this. */
        eng->has_bol = prog_has_bol(&eng->prog);
        if (!eng->has_bol) {
            int *tmp_seen  = xcalloc((size_t)eng->prog.n, sizeof(int));
            int *tmp_list  = xmalloc((size_t)eng->prog.n * sizeof(int));
            int *tmp_stk   = xmalloc((size_t)(2 * eng->prog.n + 1) * sizeof(int));
            tlist_t tmp_tl = { tmp_list, 0, 0 };
            /* pos=1: not BOL, not EOL (dummy non-boundary position) */
            addthread(&eng->prog, &tmp_tl, tmp_seen, 1, tmp_stk, 0, "", 0, 1);
            eng->init_n       = tmp_tl.n;
            eng->init_matched = tmp_tl.matched;
            eng->init_list    = xmalloc((size_t)tmp_tl.n * sizeof(int));
            memcpy(eng->init_list, tmp_list, (size_t)tmp_tl.n * sizeof(int));
            free(tmp_seen); free(tmp_list); free(tmp_stk);
        }
    }
    g_initialized = 1;
}
...
void mm17_free(void) {
    for (int p = 0; p < MM88_NUM_PATTERNS; p++) {
        free(g_engines[p].prog.code);   g_engines[p].prog.code   = NULL;
        free(g_engines[p].init_list);   g_engines[p].init_list   = NULL;
...
static size_t scan_one(int p, const char *input, size_t len,
                       mm17_match_t *out, size_t max, size_t count) {
    pat_engine_t *eng = &g_engines[p];
    prog_t       *pr  = &eng->prog;
    ...
    while (pos <= len) {
         ...
         * For patterns without BOL anchors the initial closure is position-
         * independent: copy the precomputed list directly (no DFS). For BOL
         * patterns fall back to addthread as before. */
        gen++;
        if (!eng->has_bol) {
            cl->n       = eng->init_n;
            cl->matched = eng->init_matched;
            memcpy(cl->list, eng->init_list, (size_t)eng->init_n * sizeof(int));
            /* stamp seen[] for each copied pc so addthread won't re-add them
             * when following transitions into the next closure. */
            for (int i = 0; i < eng->init_n; i++) seen[cl->list[i]] = gen;
        } else {
            cl->n = 0; cl->matched = 0;
        ...
        if (match_end != (size_t)-1) {
            ...
            if (count < max) out[count++] = (mm17_match_t){p, pos, span};
...
int main(void) {
    ...
    int pass=0;
    mm17_match_t out[256];

    for (int t=0; t<n; t++) {
        size_t len=strlen(tests[t].input);
        size_t nm=mm17_scan(tests[t].input, len, out, 256);
        int found=0, no_expect=strcmp(tests[t].expect,"(none)")==0;
        for (size_t m=0; m<nm; m++) {
            const char *pn=mm17_pattern_name(out[m].pattern_id);

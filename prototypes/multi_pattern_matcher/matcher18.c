/* matcher18.c — v18: v15.1 + per-pattern lazy DFA transition cache.
 *
 * v15.1's inner loop steps the NFA state set byte-by-byte: iterate cl->n threads,
 * dispatch per instruction, epsilon-close into the next set via addthread. v17
 * proved the seed addthread is negligible; the cost is this inner step loop.
 *
 * v18 collapses the per-byte NFA-set step into a single table lookup:
 *   dfa_state = table[dfa_state][byte]
 * Each DFA state is an interned, canonical (sorted) set of byte-consuming NFA pcs
 * plus a `matched` flag. table[state][byte] is filled LAZILY: on first access we
 * run v15's exact NFA step+closure once, canonicalize the resulting pc-set, intern
 * it to a small int id, and cache it. Subsequent visits are O(1) lookups.
 *
 * This is PER-PATTERN — each pattern has its own tiny DFA (no merge), so there is
 * no state explosion (v4's failure) and no cross-pattern contamination. The
 * leftmost-longest, non-overlapping match-emission in scan_one is UNCHANGED from
 * v15 (v9/v10 were incorrect because their reset logic was wrong, not the table).
 *
 * BOL/EOL caveat: a DFA state cannot encode "am I at a line boundary." Patterns
 * containing ^ or $ (incl. all boundary-wrapped patterns) fall back to v15's exact
 * inner loop. Only anchor-free patterns use the DFA. (Anchor lowering for the
 * boundary-wrapped digit patterns is a documented follow-up, v18.1.)
 *
 * Build:  make matcher18.so   (shared lib)
 *         make matcher18       (self-test binary)
 */
...
/* Returns 1 if the program contains any line-anchor (BOL/EOL), which makes the
 * epsilon closure position-dependent and therefore not DFA-cacheable. */
...
void mm18_init(void) {
    ...
    for (int p = 0; p < MM88_NUM_PATTERNS; p++) {
        ...
        eng->use_dfa = !prog_has_anchor(&eng->prog) && !full;
    }
    g_initialized = 1;
}
...
static int dfa_compute_trans(int p, int sid, unsigned char c) {
    ...
    for (int i = 0; i < sn; i++) {
        ...
        if (matches)
            addthread(pr, nl, seen, gen, estk, set[i] + 1, "", 0, 1);
    }
...
static void dfa_build_start(int p) {
    ...
    int gen = ++g_gen[p];
    cl->n = 0; cl->matched = 0;
    addthread(pr, cl, seen, gen, estk, 0, "", 0, 1);
...
static size_t scan_one(int p, const char *input, size_t len,
                       mm18_match_t *out, size_t max, size_t count) {
    pat_engine_t *eng = &g_engines[p];
    prog_t       *pr  = &eng->prog;
    ...
    while (pos <= len) {
        ...
        if (eng->use_dfa) {
            /* DFA inner loop: one table lookup per byte. Start state is id 0.
             * Lazily fill transitions on first encounter via dfa_compute_trans. */
            int st = 0;
            while (st != DFA_DEAD) {
            ...
            /* Fallback NFA inner loop (patterns with BOL/EOL anchors): identical
             * to v15.1. Uses the persistent g_gen[p] directly. */
            int gen = ++g_gen[p];
            cl->n = 0; cl->matched = 0;
        ...
        if (match_end != (size_t)-1) {
            ...
            if (count < max) out[count++] = (mm18_match_t){p, pos, span};
...
int main(void) {
    ...
    int dfa = 0, fb = 0;
    for (int p = 0; p < MM88_NUM_PATTERNS; p++)
        if (g_engines[p].use_dfa) dfa++; else fb++;
    printf("DFA path: %d patterns | NFA fallback (anchors): %d patterns\n\n", dfa, fb);
    ...
    int pass=0;
    mm18_match_t out[256];

    for (int t=0; t<n; t++) {
        size_t len=strlen(tests[t].input);
        size_t nm=mm18_scan(tests[t].input, len, out, 256);
        int found=0, no_expect=strcmp(tests[t].expect,"(none)")==0;
        for (size_t m=0; m<nm; m++) {
            const char *pn=mm18_pattern_name(out[m].pattern_id);

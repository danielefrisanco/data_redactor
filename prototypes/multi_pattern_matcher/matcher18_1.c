/* matcher18_1.c — v18.1: v18 + anchor lowering (all 88 patterns on the DFA path).
 *
 * v18 put 64/88 patterns on the lazy DFA path but fell back to v15.1's NFA inner
 * loop for the 24 patterns with BOL/EOL anchors (the boundary-wrapped digit/format
 * patterns: SSN, PESEL, credit card, IPv4, IBAN, etc.).
 *
 * The key insight: `OP_BOL` in the boundary wrapper `(^|[^0-9A-Za-z])` only fires
 * at position 0 (or after '\n'). For all other positions the `^` branch never
 * matches anyway — the `[^0-9A-Za-z]` branch is the only live one. So in DFA
 * transition computation (which is position-independent), treating BOL and EOL as
 * always-passable (epsilon-skip) is a SAFE SUPERSET: it may admit NFA states that
 * a real match would reject at non-boundary positions, but the actual match
 * confirmation still happens through the DFA, which correctly rejects bytes not in
 * the boundary class.
 *
 * Implementation: `addthread_dfa` — identical to `addthread` but treats OP_BOL and
 * OP_EOL as unconditionally passable (push pc+1 always). Used only when filling
 * DFA transition entries; the NFA fallback path still uses position-sensitive
 * `addthread`. With this change `prog_has_anchor` becomes irrelevant and
 * `eng->use_dfa = 1` for all 88 patterns.
 *
 * Correctness risk: the superset means the DFA start state for boundary-wrapped
 * patterns now includes pcs reachable via `^` even at pos>0 — those pcs consume
 * a byte that can only match `[^0-9A-Za-z]`, so they die on the first byte test
 * unless the byte actually is non-alphanumeric. No false match can be emitted
 * because match_end is only set when `matched` is 1 AND `sp - pos >= min_len`.
 * verify18_1.rb confirms byte-for-byte identity with v15.1.
 *
 * Build:  make matcher18_1.so   (shared lib)
 *         make matcher18_1       (self-test binary)
 */

#define _GNU_SOURCE
#include "matcher18_1.h"
...
static size_t scan_one(int p, const char *input, size_t len,
                       mm18_1_match_t *out, size_t max, size_t count) {
    pat_engine_t *eng = &g_engines[p];
    prog_t       *pr  = &eng->prog;
    ...
    while (pos <= len) {
        ...
        if (match_end != (size_t)-1) {
            ...
            if (count < max) out[count++] = (mm18_1_match_t){p, pos, span};
...
int main(void) {
    ...
    int pass=0;
    mm18_1_match_t out[256];

    for (int t=0; t<n; t++) {
        size_t len=strlen(tests[t].input);
        size_t nm=mm18_1_scan(tests[t].input, len, out, 256);
        int found=0, no_expect=strcmp(tests[t].expect,"(none)")==0;
        for (size_t m=0; m<nm; m++) {
            const char *pn=mm18_1_pattern_name(out[m].pattern_id);

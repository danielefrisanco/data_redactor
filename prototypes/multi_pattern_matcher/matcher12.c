/* matcher12.c — v12.1: v11 bytecode VM + literal pre-filter for ALL patterns.
 *
 * v11 only skipped noise for the 40 start-anchored-prefix patterns (via memmem).
 * The 48 infix-literal patterns were scanned at every byte, which is where v11's
 * ~11 ns/pattern/byte on noise came from.
 *
 * v12.1 extends the memmem skip to the infix patterns. A match must contain the
 * pattern's required literal; a match start can therefore only lie in the window
 * [hit - max_prefix .. hit] for each literal occurrence `hit`, where max_prefix
 * is the longest run of bytes the regex can consume before the literal. When
 * max_prefix is bounded (computed from the AST at init), we memmem the literal
 * and seed the VM only inside those windows. When it is unbounded (~a handful of
 * patterns, e.g. uri_with_password's [A-Za-z0-9+_-]*://), we fall back to v11's
 * every-byte scan for that pattern — correct, just not sped up.
 *
 * Everything else (parser, bytecode, VM, gsub semantics) is identical to v11.
 *
 * Build:  make matcher12.so   (shared lib)
 *         make matcher12       (self-test binary)
 */
...
static void cc_add_posix(cclass_t *c, const char *cls, size_t len) {
    ...
    } else { fprintf(stderr,"matcher12: unknown POSIX class [:%.*s:]\n",(int)len,cls); exit(1); }
}
...
static size_t scan_one(int p, const char *input, size_t len,
                       mm12_match_t *out, size_t max, size_t count) {
    pat_engine_t *eng = &g_engines[p];
    prog_t       *pr  = &eng->prog;
    ...
    while (pos <= len) {
        ...
        if (match_end != (size_t)-1) {
            ...
            if (count < max) out[count++] = (mm12_match_t){p, pos, span};
...
int main(void) {
    ...
    int pass=0;
    mm12_match_t out[256];

    for (int t=0; t<n; t++) {
        size_t len=strlen(tests[t].input);
        size_t nm=mm12_scan(tests[t].input, len, out, 256);
        int found=0, no_expect=strcmp(tests[t].expect,"(none)")==0;
        for (size_t m=0; m<nm; m++) {
            const char *pn=mm12_pattern_name(out[m].pattern_id);

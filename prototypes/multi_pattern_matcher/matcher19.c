...
static size_t scan_one(int p, const char *input, size_t len,
    ...
    while (pos <= len) {
        ...
        if (match_end != (size_t)-1) {
            ...
            if (count < max) out[count++] = (mm19_match_t){p, pos, span};
...
static size_t scan_digit_group(const char *input, size_t len,
    ...
    while (i < len) {
        ...
        for (int p = 0; p < MM88_NUM_PATTERNS && count < max; p++) {
            ...
            out[count++] = (mm19_match_t){p, start, end - start};
...
int main(void) {
    ...
    int pass=0;
    mm19_match_t out[256];

    for (int t=0; t<n; t++) {
        size_t len=strlen(tests[t].input);
        size_t nm=mm19_scan(tests[t].input, len, out, 256);
        int found=0, no_expect=strcmp(tests[t].expect,"(none)")==0;
        for (size_t m=0; m<nm; m++) {
            const char *pn=mm19_pattern_name(out[m].pattern_id);

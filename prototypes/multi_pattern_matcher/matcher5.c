/* Multi-matcher prototype v5 — AC trie + Onigmo + Boyer-Moore infix filter.
 *
 * Same two-stage architecture as matcher3.c (v3) but adds a Boyer-Moore
 * shift-table pre-filter for always-candidate patterns that have a known
 * required literal substring (the `bm_literal` field in patterns_generated.h).
 *
 * For those patterns, Stage 2 uses BM to skip over positions where the
 * required literal cannot land, then calls onig_search only from each
 * candidate anchor position.  Patterns without a bm_literal fall back to
 * plain onig_search, identical to v3.
 *
 * Build:  make matcher5.so
 * Smoke:  make matcher5 && ./matcher5
 */
...
/*
 * Classic BM bad-character shift table (256 entries, one per input byte).
 * shift[c] = distance to advance when byte c appears at the mismatch position.
 * Pre-computed per-pattern at init time for patterns that have a bm_literal.
 *
 * We use only the bad-character rule (no good-suffix) — simple and sufficient
 * for the short literals we have here.
 */
typedef struct {
    size_t shift[256];
    size_t pat_len;
    const char *pat;  /* pointer into MM88_PATTERNS[p].bm_literal */
} bm_table_t;
...
static void bm_build(int p) {
    ...
    /* Default shift = pattern length (bad character not in pattern) */
    for (int c = 0; c < 256; c++) g_bm[p].shift[c] = m;
    /* For each character in the pattern (except the last), set shift = distance
     * from that position to the end of the pattern. */
    for (size_t i = 0; i < m - 1; i++)
        g_bm[p].shift[(unsigned char)lit[i]] = m - 1 - i;
}

/*
 * BM search: find the first occurrence of g_bm[p].pat in [haystack, haystack+len).
 * Returns pointer to match start, or NULL if not found.
 *
 * We search right-to-left within each window (standard BM), but since we only
 * use the bad-character rule the last character drives the skip.
 */
static const char *bm_search(int p, const char *haystack, size_t len) {
    const bm_table_t *bm = &g_bm[p];
    const char *pat = bm->pat;
    size_t m = bm->pat_len;
    if (!pat || m == 0) return haystack;  /* no filter */
...
/* ---------- Aho-Corasick (identical to matcher3.c) ---------------------- */
...
const char *mm5_pattern_name(int id) {
    if (id < 0 || id >= MM88_NUM_PATTERNS) return "<invalid>";
    return MM88_PATTERNS[id].name;
}

/* ---------- Confirmation via Onigmo (Stage 1, identical to v3) ---------- */
...
size_t mm5_scan(const char *input, size_t len,
    ...
    /* Stage 1: prefix-filtered patterns (identical to v3). */
    ...
    /* Stage 2: always-candidate patterns with BM pre-filter.
     *
     * For patterns with a bm_literal: scan the whole input with BM to find
     * each occurrence of the literal.  At each hit, call onig_search starting
     * from max(0, hit_pos - max_regex_len) to find the actual match that
     * contains this literal.
     *
     * For patterns without a bm_literal: fall back to plain onig_search
     * (identical to v3 Stage 2).
     *
     * max_regex_len is an upper bound on how far before the literal the match
     * can start.  We use a generous 4096 to cover all our patterns.
     */
    for (int p = 0; p < MM88_NUM_PATTERNS; p++) {
        ...
        if (!bm_lit || g_bm[p].pat_len == 0) {
            /* No BM literal — plain onig_search (same as v3). */
            ...
            /* BM pre-filter: find each occurrence of bm_literal, then
             * call onig_search anchored to a window around the hit. */
            ...
            while (remaining > 0) {
                ...
                /* The literal landed at `hit`. The pattern match must include
                 * this literal, so the match start is at most `hit - 0` bytes
                 * before `hit` (for patterns where literal is at the end) and
                 * at most `match_max_len` bytes before.  We use a window of
                 * [hit - 4096, hit + lit_len + 4096] clamped to [0, len). */
                ...
                const OnigUChar *search_from = str + window_start;
                const OnigUChar *search_end  = str + window_end;

                OnigRegion *region = onig_region_new();
                if (!region) break;

                int rc = onig_search(g_onig[p], str, end, search_from,
                                     search_end, region, ONIG_OPTION_NONE);
                if (rc < 0) {
                    onig_region_free(region, 1);
                    /* No match containing this literal — advance past hit. */
...
int main(void) {
    ...
    mm5_match_t buf[1024];
    for (int i = 0; cases[i]; i++) {
        size_t n = mm5_scan(cases[i], strlen(cases[i]), buf, 1024);
        printf("[%d] %zu matches: %s\n", i, n, cases[i]);
        for (size_t j = 0; j < n && j < 8; j++) {
            printf("    %-30s pos=%zu len=%zu  '%.*s'\n",
                   mm5_pattern_name(buf[j].pattern_id),

...
void mm19c_init(void) {
    ...
    for (int p = 0; p < MM88_NUM_PATTERNS; p++) {
        ...
        /* v19c: patterns whose regex literally begins with "https://" carry only an
         * *infix* bm_literal and an unbounded max_len, so the skip is disabled and
         * they full-scan to every 'h'. The literal prefix is itself a perfectly good
         * start-anchored req_literal — prefer it, enabling memmem skip with max_back=0
         * and no separate scan path. (Cheaper alternative to v19b's scan_https_group.) */
        if (!pd->boundary_wrapped &&
            strncmp(pd->regex, "https://", 8) == 0) {
            eng->req_literal      = "https://";
            eng->req_lit_len      = 8;
            eng->req_lit_at_start = 1;
        } else if (pd->prefix && !pd->prefix_is_infix) {
...
static size_t scan_one(int p, const char *input, size_t len,
                       mm19c_match_t *out, size_t max, size_t count) {
    pat_engine_t *eng = &g_engines[p];
    prog_t       *pr  = &eng->prog;
    ...
    while (pos <= len) {
        ...
        if (match_end != (size_t)-1) {
            ...
            if (count < max) out[count++] = (mm19c_match_t){p, pos, span};
...
static size_t scan_digit_group(const char *input, size_t len,
    ...
    while (i < len) {
        ...
        for (int p = 0; p < MM88_NUM_PATTERNS && count < max; p++) {
            ...
            out[count++] = (mm19c_match_t){p, start, end - start};
...
static size_t scan_iban_group(const char *input, size_t len,
    ...
    while (i + 1 < len && count < max) {
        ...
        if (match_end != (size_t)-1) {
            ...
            out[count++] = (mm19c_match_t){p, i, span};
...
size_t mm19c_scan_range(int lo, int hi, const char *input, size_t len,
                       mm19c_match_t *out, size_t max) {
    size_t count = 0;
...
int main(void) {
    ...
    int pass=0;
    mm19c_match_t out[256];

    for (int t=0; t<n; t++) {
        size_t len=strlen(tests[t].input);
        size_t nm=mm19c_scan(tests[t].input, len, out, 256);
        int found=0, no_expect=strcmp(tests[t].expect,"(none)")==0;
        for (size_t m=0; m<nm; m++) {
            const char *pn=mm19c_pattern_name(out[m].pattern_id);

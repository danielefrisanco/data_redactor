/* matcher15.c — v15.1: v14 (literal + first-byte filters) + VM constant-factor.
 *
 * v14 is already O(N) and allocation-free on realistic payloads (~75 ms/100KB on
 * dense). v15.1 keeps v14's structure and semantics unchanged and only lowers the
 * per-byte constant of the Thompson VM inner loop:
 *
 *   1. Iterative addthread — replace the recursive epsilon-closure with an
 *      explicit fixed stack (no call overhead, no stack-depth risk).
 *   2. Inline the hot CHAR step and keep the CLASS bitmap test branch-light.
 *
 * No algorithmic change: same leftmost-longest, non-overlapping gsub semantics,
 * same per-start-position scan, same O(N) on real input / O(N²) adversarial worst
 * case (the worst case is v15.2's target, the single-pass rewrite). Verified
 * against the independent per-pattern gsub reference exactly as v14.
 *
 * Allocation discipline: unchanged from v14 — fixed per-pattern bitmaps + bytecode
 * at init, reused scan scratch, zero allocation in the scan hot path.
 *
 * Build:  make matcher15.so   (shared lib)
 *         make matcher15       (self-test binary)
 */
...
static size_t scan_one(int p, const char *input, size_t len,
                       mm15_match_t *out, size_t max, size_t count) {
    pat_engine_t *eng = &g_engines[p];
    prog_t       *pr  = &eng->prog;
    ...
    while (pos <= len) {
        ...
        if (match_end != (size_t)-1) {
            ...
            if (count < max) out[count++] = (mm15_match_t){p, pos, span};
...
int main(void) {
    ...
    int pass=0;
    mm15_match_t out[256];

    for (int t=0; t<n; t++) {
        size_t len=strlen(tests[t].input);
        size_t nm=mm15_scan(tests[t].input, len, out, 256);
        int found=0, no_expect=strcmp(tests[t].expect,"(none)")==0;
        for (size_t m=0; m<nm; m++) {
            const char *pn=mm15_pattern_name(out[m].pattern_id);

/* matcher14.c — v14: v12.1 (literal pre-filter) + per-pattern first-byte filter.
 *
 * v12.1 showed a literal filter only helps the ~30 literal-bearing patterns; the
 * ~40 literal-less boundary-wrapped digit-ID patterns (SSN, credit_card, IPv4,
 * PESEL, …) scan every byte and dominate the cost (~920 ms/iter on 1 MB).
 *
 * v14 adds a first-byte filter that DOES reach those patterns. Because the engine
 * is constrained to pure POSIX-ERE (no \d\w\s\b, lookaround, named groups,
 * non-greedy — enforced in lib/data_redactor.rb), every pattern is a true regular
 * language with a well-defined set of bytes that can be its first *consumed* byte.
 * We compute that 256-bit set once at init from the bytecode start closure, then
 * in scan_one skip forward over any run of input bytes that the set rejects —
 * entirely in C, never entering the VM. Boundary-wrapped digit patterns start
 * with (^|[^0-9A-Za-z]), so their first-byte set is the non-alphanumerics, which
 * still skips the alphanumeric noise runs.
 *
 * The literal skip from v12.1 is kept and composes with the first-byte filter.
 *
 * Allocation discipline: first[] is a fixed 256-bit bitmap per pattern, filled
 * once at init. The scan hot path allocates nothing (reuses the v11 scratch).
 *
 * Build:  make matcher14.so   (shared lib)
 *         make matcher14       (self-test binary)
 */
...
static void cc_add_posix(cclass_t *c, const char *cls, size_t len) {
    ...
    } else { fprintf(stderr,"matcher14: unknown POSIX class [:%.*s:]\n",(int)len,cls); exit(1); }
}
 ...
 * the scan hot path allocates nothing. */
typedef struct {
    int      *list;   /* pc list */
    int       n;
} tlist_t;
...
static size_t scan_one(int p, const char *input, size_t len,
                       mm14_match_t *out, size_t max, size_t count) {
    pat_engine_t *eng = &g_engines[p];
    prog_t       *pr  = &eng->prog;
    ...
    while (pos <= len) {
        ...
        if (match_end != (size_t)-1) {
            ...
            if (count < max) out[count++] = (mm14_match_t){p, pos, span};
...
int main(void) {
    ...
    int pass=0;
    mm14_match_t out[256];

    for (int t=0; t<n; t++) {
        size_t len=strlen(tests[t].input);
        size_t nm=mm14_scan(tests[t].input, len, out, 256);
        int found=0, no_expect=strcmp(tests[t].expect,"(none)")==0;
        for (size_t m=0; m<nm; m++) {
            const char *pn=mm14_pattern_name(out[m].pattern_id);
